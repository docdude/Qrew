...
class QrewMessageBox(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.message_label.setStyleSheet('font-size: 14px; color: white; line-height: 1.4;')
    ...
    def _set_icon(self, msg_type):
        ...
        self.icon_label.setText(f'<span style="color: {color}; font-size: 50px;">{icon}</span>')
        self.title_label.setStyleSheet(f'font-weight: bold; font-size: 16px; color: {color};')
        
    def _add_button(self, text, button_type, style_name='secondary'):
        """Add a button to the dialog"""
        button = Button(text)
        button.setStyleSheet(BUTTON_STYLES.get(style_name, BUTTON_STYLES['secondary']))
...
class QrewFileDialog(QDialog):
    ...
    def __init__(self, parent=None, mode='open', caption='Select File', 
    ...
    @classmethod
    def getOpenFileName(cls, parent=None, caption='Open File', 
                        directory='', filter='All Files (*.*)', 
                        default_suffix=''):
        """Class method to get open file name"""
        dialog = cls(parent, mode='open', caption=caption, 
    ...
    @classmethod
    def getSaveFileName(cls, parent=None, caption='Save File', 
                        directory='', filter='All Files (*.*)', 
                        default_suffix=''):
        """Class method to get save file name"""
        dialog = cls(parent, mode='save', caption=caption, 
                     directory=directory, filter=filter, 
                     default_suffix=default_suffix)
        if dialog.exec_():
            return dialog.selected_file
