...
class MainWindow(QMainWindow):
    def __init__(self):
       ...
       # self.settings_btn.setIcon(QIcon("gear@2x.png"))
        ...
        # Checkbox grid with proper spacing
        ...
        for col in range(4):
            checkbox_layout.setColumnStretch(col, 1)
...
#        main_layout.addWidget(self.status_label, alignment=Qt.AlignCenter)
    ...
    def _rebuild_grid(self, text):
        ...
        was_flashing = hasattr(self, 'flash_timer') and self.flash_timer and self.flash_timer.isActive()
        
        # Get the container
        parent_widget = self.grid_widget.parent()
        parent_layout = parent_widget.layout()
        
        # Remove old widget
        parent_layout.removeWidget(self.grid_widget)
    ...
    def toggle_flash(self):
        """Toggle flash state"""
        current_flash = self.grid_widget.flash_on
        self.grid_widget.set_flash(not current_flash)

    def update_grid_position(self, position):
    ...
    def handle_repeat_measurements(self, selected_measurements):
        ...
        self.start_button.setEnabled(False)
        self.status_label.setText('Starting remeasurement process...')
    ...
    def apply_settings(self):
     ...
     #   qs.get('show_vlc_gui', False)
      #  qs.get('vlc_backend', 'auto')
        qs.get('viz_view', "Grid View")


    def switch_visualization_mode(self, mode):
        """Switch between grid, compact, and full visualization modes"""
        if mode == "Grid View":
            # Show grid widget, hide compact mic widget
            if self.grid_widget:
                self.grid_widget.show()
            if self.compact_mic_widget:
                self.compact_mic_widget.hide()
            if self.visualization_dialog:
                self.visualization_dialog.hide()
                
        elif mode == "Compact Theater View":
            # Hide grid, show compact mic widget
            if self.grid_widget:
                self.grid_widget.hide()
            self.show_compact_visualization()
            if self.visualization_dialog:
                self.visualization_dialog.hide()
                
        elif mode == "Full Theater View":
            # Show dialog version
            self.show_visualization_dialog()
            # Optionally keep grid or compact view visible
            
    def show_compact_visualization(self):
        if not self.compact_mic_widget:
            grid_container = self.grid_widget.parent()
            self.compact_mic_widget = MicPositionWidget('/Users/juanloya/Documents/qrew/qrew/hometheater_base_persp.png', '/Users/juanloya/Documents/qrew/qrew/room_layout_persp.json')
            
            # Scale for compact view
            self.compact_mic_widget.set_scale(0.25)  # 25% of original size
            
            grid_container.layout().addWidget(self.compact_mic_widget)
    ...
    def update_mic_visualization(self):
        ...
        if self.compact_mic_widget and self.compact_mic_widget.isVisible():
            self.compact_mic_widget.set_active_mic(current_pos)
            self.compact_mic_widget.set_active_speakers(active_speakers)
            
        # Update dialog
        if self.visualization_dialog and self.visualization_dialog.isVisible():
            self.visualization_dialog.update_visualization(current_pos, active_speakers)

    def update_visualization_from_worker(self, position, active_speakers):
        """Update visualization from worker thread"""
        if self.compact_mic_widget and self.compact_mic_widget.isVisible():
            self.compact_mic_widget.set_active_mic(position)
            self.compact_mic_widget.set_active_speakers(active_speakers)
            
        if self.visualization_dialog and self.visualization_dialog.isVisible():
            self.visualization_dialog.update_visualization(position, active_speakers)
    ...
    def create_floating_viz_button(self):
        """Create a floating button to quickly toggle visualization"""
        self.viz_float_button = QToolButton(self)
        self.viz_float_button.setIcon(QIcon("icons/theater_view.png"))  # You'll need an icon
        self.viz_float_button.setIconSize(QSize(32, 32))
        self.viz_float_button.setToolTip("Toggle Theater Visualization")
        self.viz_float_button.clicked.connect(self.toggle_visualization)
        self.viz_float_button.setStyleSheet("""
            QToolButton {
        ...
        self.viz_float_button.move(self.width() - 50, 10)
        self.viz_float_button.raise_()
        
    def toggle_visualization(self):
        """Quick toggle between grid and theater view"""
        current_mode = self.viz_mode_combo.currentText()
        if current_mode == "Full Theater View":
            self.viz_mode_combo.setCurrentText("Grid View")
        else:
            self.viz_mode_combo.setCurrentText("Full Theater View")
            
    def resizeEvent(self, event):
        """Keep floating button in corner on resize"""
        super().resizeEvent(event)
        if hasattr(self, 'viz_float_button'):
            self.viz_float_button.move(self.width() - 50, 10)
    ...
    def connect_visualization_signals(self):
        ...
        for checkbox in self.channel_checkboxes.values():
            checkbox.stateChanged.connect(self.update_channel_visualization)
    ...
    def update_channel_visualization(self):
        ...
        if hasattr(self, 'compact_mic_widget') and self.compact_mic_widget:
            self.compact_mic_widget.set_active_speakers(selected_channels)
        ...
        if hasattr(self, 'visualization_dialog') and self.visualization_dialog:
            self.visualization_dialog.update_visualization(
                active_speakers=selected_channels
            )
