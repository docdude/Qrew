...
class MeasurementQualityDialog(QDialog):
    def __init__(self, measurement_info, parent=None):
        ...
        icon_label = QLabel(f'<span style="color: {color}; font-size: 24px;">{icon}</span>')
        icon_label.setTextFormat(Qt.RichText)
        title_layout.addWidget(icon_label)
        
        title_label = QLabel(title_text)
        title_label.setStyleSheet(f'font-weight: bold; font-size: 16px; color: {color};')
        ...
        # Message
        message = f"""
        The measurement for <b>{channel}_pos{position}</b> received a quality rating of <b>{rating}</b> 
        with a score of <b>{score:.1f}</b>, <b>{detail_str}<b>.<br><br>
...
class SaveMeasurementsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        title_label.setStyleSheet('font-weight: bold; font-size: 18px; margin-bottom: 15px;')
     ...
     #   self.browse_button.setFixedWidth(100)
      #  self.browse_button.setMinimumHeight(30)
        ...
        # Info section
        info_label = QLabel("This will save all raw measurements before any processing "
...
class ClearMeasurementsDialog(QDialog):
    def __init__(self, measurement_count, parent=None):
        ...
        title_label.setStyleSheet('font-weight: bold; font-size: 16px; color: #d32f2f;')
        title_layout.addWidget(title_label)
        title_layout.addStretch()
        ...
        if measurement_count > 0:
            message_text = f'REW currently contains {measurement_count} measurement{"s" if measurement_count != 1 else ""}.\n\n' \
                          'Do you want to delete all existing measurements before starting new ones?\n\n' \
                          'This action cannot be undone.'
        else:
            message_text = 'REW contains no existing measurements.\n\nProceed with new measurements?'
...
class RepeatMeasurementDialog(QDialog):
    def __init__(self, measurement_qualities, num_positions, parent=None):
        ...
        title_label.setStyleSheet('font-weight: bold; font-size: 16px; margin-bottom: 0px;')
        layout.addWidget(title_label)
    ...
    def create_quality_summary(self, layout):
        ...
        for (channel, position), quality in self.measurement_qualities.items():
            ...
            score = quality['score']
            if rating == 'RETAKE':
                retake_measurements.append(f"{channel}_pos{position} (Score: {score:.1f})")
            elif rating == 'CAUTION':
        ...
        if retake_measurements:
            ...
            retake_label.setStyleSheet('color: #ff0000; font-weight: bold; padding-bottom: 0px;')
    ...
    def create_channel_selection(self, layout):
        ...
        available_channels = set(channel for channel, pos in self.measurement_qualities.keys())
    ...
    def create_position_selection(self, layout):
        ...
        available_positions = set(pos for channel, pos in self.measurement_qualities.keys())
   ...
   #     self.cancel_button.setFixedSize(100, 35)
    ...
    def update_selection(self):
        ...
        selected_positions = [pos for pos, cb in self.position_checkboxes.items() if cb.isChecked()]
        ...
        if self.selected_measurements:
            ...
            for measurement in self.selected_measurements:
                ...
                if measurement['rating'] == 'RETAKE':
                    ...
                    icon = f'<span style="color: #888;">{HTML_ICONS["circle_gray"]}</span>'
                line = (f'{icon} {measurement["channel"]}_pos{measurement["position"]} '
...
class DeleteSelectedMeasurementsDialog(QDialog):
    def __init__(self, selected_measurements, parent=None):
        ...
        # Message
        message_text = f'The following {len(selected_measurements)} measurements will be deleted:\n\n'
        for measurement in selected_measurements:
            message_text += f"• {measurement['channel']}_pos{measurement['position']} "
            message_text += f"({measurement['rating']}, Score: {measurement['score']:.1f}) "
            message_text += f"(UUID: {measurement['uuid']})\n"
        message_text += '\nThis action cannot be undone.'
        
        message_text += '\nThese measurements will be permanently removed before remeasuring.\n\nProceed?'
